...
- Репозиторий представляет собой черновой рабочий контур для распознавания РЖЯ, в котором одновременно сосуществуют несколько разных пайплайнов: `letters`, `words` и `pose_words`.
...
- Верхний `README.md`, `backend/README.md` и архитектурные документы теперь синхронизированы по базовой картине: это draft-репозиторий с режимами `letters`, `words` и `pose_words`, где `pose_words` является целевым pipeline для слов, а `words` сохраняется как baseline/reference.
...
- готовность `pose_words` как продуктового режима даже при наличии supported active non-dummy runtime path.
